...
class MeshDrawer {
	...
	constructor() {
		...
		this.normbuffer=gl.createBuffer();//this is for the normla buffer
		...
		this.enableLighthingLoc=gl.getUniformLocation(this.prog,'enableLighting');
	...
	draw(trans) {
		...
		gel.uniform3fv(this.lightPosLoc,[lightX,lightY,0]);
		updateLightPos();
	...
	setTexture(img) {
		...
		if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
			...
			gl.texParemeteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);
			gl.texParemeteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);
			gl.texParemeteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);
			gl.texParemeteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);
	...
	setAmbientLight(ambient) {
		//this one enables or disables the lighting
		gl.useProgram(this.prog);
		gl.uniform1i(this.enableLighthingLoc,ambient);
	}
}
...
const meshFS = `
			...
			void main()
			{
				if(showTex && enableLighting){
					...
					//this is for the ambient lighting
					vec3 ambientLight=ambient*vec3(texture2d(tex,v_texCoord));
					//diffusing the light
					vec3 norm=normalive(v_normal);
